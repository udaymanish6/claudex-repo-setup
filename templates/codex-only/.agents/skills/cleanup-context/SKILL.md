...
description: Use when context is bloated, stale, or needs compacting into durable project memory before continuing.
---
...
Analyze the project's Claude memory files (AGENT-*.md and CLAUDE.md) to identify and eliminate token waste through:
...
find . -name "AGENT-*.md" -exec wc -c {} \; | sort -nr
wc -c CLAUDE.md AGENT-*.md 2>/dev/null
```
...
3. Add archive index with retained context pointers
...
find . -name "AGENT-*.md" -not -path "*/archive/*" -exec wc -c {} \; | awk '{sum+=$1} END {print sum}'
```
